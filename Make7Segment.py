...
def load_profile(path):
    ...
    boolean_keys = set([
        'show_board_dim', 'debug_centers', 'allow_vertical_overlap', 'pause_after_seg', 'debug_gcode', 'use_shapely', 'no_shapely', 'rotate', 'pocket_middle'
    ])
...
def write_gcode(filename, W, H, segments, bit_dia=3.0, cut_depth=3.0, pass_depth=1.5, feed=800, plunge=300, safe_z=5.0, show_board=False, board_w=None, board_h=None, margin=10, rotate=False, spindle_speed=None, pause_after_segment=False, debug_gcode=False, pocket_middle=False, pocket_step=None):
    with open(filename, 'w') as f:
        if debug_gcode:
        ...
        if show_board and (board_w is not None) and (board_h is not None):
            ...
            for seg in segments:
                ...
                for pi in range(passes):
                    ...
                    if pocket_middle:
                        # decide horizontal scanline spacing
                        ...
                        try:
                            if _HAS_SHAPELY:
                                ...
                                while y <= maxy:
                                    ...
                                    y += step
                                # execute each scanline segment as a cut at target_depth
                                for coords in lines:
                                    sx, sy = coords[0]
                                    ex, ey = coords[-1]
                                    f.write(f"G0 Z{fr(safe_z)}\n")
                                ...
                                # fallback: sample points along scanline and detect inside runs
